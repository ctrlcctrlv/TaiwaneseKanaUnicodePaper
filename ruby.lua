...
local rubyFont = defaultRubyFont
SILE.registerCommand("ruby:font", function (options, _)
  if options then
    for k, v in pairs(options) do rubyFont[k] = v end
  end
  SILE.call("font", rubyFont)
end)

local defaultRubyHeight = SILE.measurement("1zw")
local rubyHeight = defaultRubyHeight
SILE.registerCommand("ruby:set", function (options, _)
  rubyHeight = SILE.measurement(options.height)
end)
...
SILE.registerCommand("ruby", function (options, content)
  ...
  rubybox.outputYourself = function (self, typesetter, line)
    ...
    typesetter.frame:advancePageDirection(-rubyHeight)
